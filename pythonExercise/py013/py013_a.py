...
def get_pic_urls(text):
    ...
    jandan_pattern = re.compile('a href="//(\S*.jpg)" target="_blank"', re.IGNORECASE)
    urls = re.findall(jandan_pattern, text)
    return urls
...
if __name__ == '__main__':
    ...
    jandan_url = "http://jandan.net/pic/page-148"
    # data = get_pic_page(url)
    data_a = get_pic_page(jandan_url)
    print("jandan_text --->{}".format(data_a))
    # # print("text --->".format(data))
    urls = get_pic_urls(data_a)
    for i in urls:
        # time.sleep(0.4)
